...
#define  SEC_LPN      sections[12]            /* Lumped Parameters info       */
...
static char *sections[] = { 
  ...
  "DRIVE FUNCT", "TIME DEPENDENT", "LUMPED PARAMETERS"};
#ifdef ADR
	static int nspecs;
	static int bcADR = 0;
#endif
...
#ifdef PBC_1D
void PBC1D::ReadLPN(FILE *fp, Domain *omega){
  int i, j, nmod;
	int NoutRCR, NoutCor;
	int Nout = iparam("NOUTLETS");
  char buf[BUFSIZ], *p;

  if(findSection(SEC_LPN,buf,fp)){
    fgets(buf,BUFSIZ,fp);
    if(strstr(buf,"RCR")){
    	fgets(buf,BUFSIZ,fp);
			sscanf(buf,"%d",&NoutRCR);
			omega->IMPBC[0].NoutRCR = NoutRCR;
	    /* set-up RCR boundary condition */
  	  omega->IMPBC[0].R1 = new double[NoutRCR*3];  // proximal resistance
    	omega->IMPBC[0].C1 = omega->IMPBC[0].R1+NoutRCR;   // capacitance
    	omega->IMPBC[0].R2 = omega->IMPBC[0].C1+NoutRCR;   // distal resistance
    	memset(omega->IMPBC[0].R1,'\0',NoutRCR*3*sizeof(double));
      for (i = 0; i < NoutRCR; ++i){
				fgets(buf,BUFSIZ,fp);
        sscanf(buf,"%lf %lf %lf",&omega->IMPBC[0].R1[i],&omega->IMPBC[0].C1[i],&omega->IMPBC[0].R2[i]);
			}
		}
    fgets(buf,BUFSIZ,fp);
    if(strstr(buf,"Cor")||strstr(buf,"cor")){
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%d",&NoutCor);
			omega->IMPBC[0].NoutCor = NoutCor;
      /* set-up coronary boundary condition */
      omega->IMPBC[0].Ra = new double[NoutCor*6];
      omega->IMPBC[0].Ca = omega->IMPBC[0].Ra+NoutCor;
      omega->IMPBC[0].Ra_mic = omega->IMPBC[0].Ca+NoutCor;
			omega->IMPBC[0].Cim = omega->IMPBC[0].Ra_mic+NoutCor;
			omega->IMPBC[0].Rv = omega->IMPBC[0].Cim+NoutCor;
			omega->IMPBC[0].Pim_scal = omega->IMPBC[0].Rv+NoutCor;
      memset(omega->IMPBC[0].Ra,'\0',NoutCor*6*sizeof(double));
      for (i = 0; i < NoutCor; ++i){
        fgets(buf,BUFSIZ,fp);
        sscanf(buf,"%lf %lf %lf %lf %lf %lf",&omega->IMPBC[0].Ra[i],&omega->IMPBC[0].Ca[i],&omega->IMPBC[0].Ra_mic[i],
							&omega->IMPBC[0].Cim[i],&omega->IMPBC[0].Rv[i],&omega->IMPBC[0].Pim_scal[i]);
      }
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%d",&nmod);
      omega->IMPBC[0].Pim_nmodes = nmod;
      fgets(buf,BUFSIZ,fp);
      sscanf(buf,"%lf",&omega->IMPBC[0].Pim_wnum);

      omega->IMPBC[0].Pim_ccos = dvector(0,nmod-1);
      omega->IMPBC[0].Pim_csin = dvector(0,nmod-1);
      for(j = 0; j < nmod; ++j){
    		fgets(buf,BUFSIZ,fp);
    		sscanf(buf,"%lf %lf",omega->IMPBC[0].Pim_ccos + j,
         omega->IMPBC[0].Pim_csin + j);
      }
		}
	}
  omega->IMPBC[0].flowrate_RCR_old = new double[Nout];
  memset(omega->IMPBC[0].flowrate_RCR_old,'\0',Nout*sizeof(double));
}
#endif
