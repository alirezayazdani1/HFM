...
void Element::Iprod_3d(Element *, Basis *, Basis *, Basis *){ERR;}
...
void Tri::HelmHoltz(Metric *lambda){
  ...
  if (SVV && type != 'p') {
	...
	SVVfactor = 1.0;
      ...
      fprintf(stdout,"\nUsing Spectrally Vanishing Viscosity:\n"
...
void Prism::HelmHoltz(Metric *lambda){
  ...
  if (SVV && type != 'p') {
    register int i,j,k,cnt;
    int    fl = face[0].l, el;
    double Qa, Qb, Qc, *filter = dvector(0,Nmodes-1);
    static double  Epsilon;
    static int NCutMT, NCutM, init=1;
    ...
    // set up filter vector to scale weak Laplacian
    memset(filter,'\0',Nmodes*sizeof(double));
    ...
    for (i = 0; i < lmax; i++){
      for (j = 0; j < lmax; j++){
        if( j>= NCutM)
          ...
          Qb = 0.0;
        for (k=0; k< lmax-i;k++,++cnt){
          Qc=0;
          if( i+k>= NCutM)
            Qc = setQfactor ( i+k, lmax, NCutM, NCutMT);
          if( i+k >= NCutM || j>= NCutM )  {
            Qb= (i+k>j?Qc:Qb);
            filter[cnt] = exp(-(Qb));
          }
        }
    ...
    dcopy(qt,u1,1,ftmp1,1);
    Ofwd(ftmp1,ftmp2,lmax);
    dvmul(Nmodes,filter,1,ftmp2,1,ftmp2,1);
    Obwd(ftmp2,ftmp1,lmax);
    dcopy(qt,ftmp1,1,str,1);
...
//apply spectral filter  
    ...
    register int i,j,k,cnt;
    int    fl = face[0].l, el;
    double Qa, Qb, Qc, *filter = dvector(0,Nm-1);
    static double  Epsilon;
    static int NCutMT, NCutM, init=1;
    ...
    for (i = 0; i < lmax; i++){
      ...
      for (j = 0; j < lmax; j++){
        ...
        if( j>= NCutM)
          Qb = setQfactor ( j, lmax, NCutM, NCutMT);
        for (k=0; k< lmax;k++,++cnt){
          Qc=0;
          if( k>= NCutM)
            Qc = setQfactor ( k, lmax, NCutM, NCutMT);
    ...
    form_diprod(str,str+qt,str+2*qt,NULL);
