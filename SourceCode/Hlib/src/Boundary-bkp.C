...
// 	for(i = 0; i < qt; i++) {
// 		if (abs(X.y[i]+6.0)<1.e-6 && nbdry->elmt->type=='v' ) {
// 			nbdry->usrtype = 'j';
// 		}
// 	}
...
void Tet::update_bndry(Bndry *Bc, int save){
 ...
 vector_set(1,&(vert[vnum(fac,2)].x),&(vert[vnum(fac,2)].y),
  ...
  GetFaceCoord(Bc->face,&X);
	t_cyl = dparam("t") - floor(dparam("t")/Tp)*Tp;
	if (Bc->usrtype) {
		for(i = 0; i < qt; i++) {
		        if( Dc <1e-8) {
			   f[i]=0.033*(dparam("BR"));
