...
class PDR_Model{
 ...
 public:
  ...
  void Read_Particle_FieldFile_Init(FILE* fin,int &step,double &time){
    char buf[BUFSIZ];
    int pid,bid,pSC,pSN,pPBP,pPBW,pb,pnum,puid_cnt;
    double pST,px,py,pz;
    fgets(buf, BUFSIZ, fin);
...
int main(){
      double time;
      int step,pnum,puid_cnt;
      FILE    *fin;
      char    fname[FILENAME_MAX];
      PDR_Model* BioM = new PDR_Model();
      
      //initialization
      sprintf(fname,"aps.fcm.chk.%d",0);
      if ((fin = fopen(fname,"r")) == (FILE *) NULL)
	{fprintf(stderr,"FCM: restart file %s not found",fname); exit(-1);}
      BioM->Read_Particle_FieldFile_Init(fin,step,time);
      fclose(fin);
      return 1;
}
