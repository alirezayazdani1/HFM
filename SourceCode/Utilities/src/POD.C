...
static int  setup (FileList *f, Element_List **U, Element_List **V, Element_List **W, int *nftot, int Nsnapshots, Bndry *Ubc);
...
double  compute_InnerProduct(Element_List **U, Element_List **V );
...
int SVD(int NSnapshots, double **C, double *EIG_VAL);
...
{
  register int  i,j,k,ig;
  int       dump=0,nfields,nftot,nf;
  double alpha;
  ...
  U  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
  V  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
  Ug  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
  Vg  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
  Mean  = (Element_List **) malloc(_MAX_FIELDS*sizeof(Element_List *));
    
  nfields = setup (&f, U,V, Mean, &nftot, Nsnapshots,Ubc);
    
    sprintf(fname, "%s.fld", f.rea.name);
    // ------------ Calculaing the Time-averaged field ----------
     fprintf(stdout, "Calculating the mean ....\n");   
    for(nf = 0; nf < nfields; ++nf){
        Mean[nf]->Trans(Mean[nf],J_to_Q);
         Mean[nf]->zerofield();
       }

    alpha = 1.0/Nsnapshots;
    for (i = 0; i < Nsnapshots; ++i){
        fprintf(stdout, "Reading snapshot %d\n",i);
        sprintf(f.in.name, "%s_%d.fld", f.rea.name,i);
        ...
        for(nf = 0; nf < nfields; ++nf){
            U[nf]->Trans(U[nf],J_to_Q);
        }
    
        for(nf = 0; nf < nfields; ++nf){
        dsvtvp(U[0]->htot, alpha,U[nf]->base_h, 1, Mean[nf]->base_h, 1,Mean[nf]->base_h, 1 );
       }
}
    // ------------ Writing out the mean field ----------
   for(nf = 0; nf < nfields; ++nf)
        Mean[nf]->Trans(Mean[nf],Q_to_J);
    
    // ------------ Writeing out POD files ------------
    //sprintf (fname, "/Users/hbabaee/NektarSPM/Utilities/runs/Mean.rst");
    sprintf(fname, "%s_MEAN.fld", f.rea.name);
    fp[1] = fp[0] = fopen(fname,"w");
    
    Writefld (fp, "Omega", 0, 0.0, 3, Mean);
    fprintf(stdout,"Writing out Mean : %d .......\n",i);
    fclose(fp[0]);
  for(nf = 0; nf < nfields; ++nf)
        Mean[nf]->Trans(Mean[nf],J_to_Q);
//   ------------------------------------------------------
//   ------------- Allocating memory for Ug ---------------
//   ------------------------------------------------------    
    fprintf(stdout,"Allocating memory for group snapshots ....\n");
    for (k =0 ; k < N_Group; ++k) {
        fprintf(stdout," k= %d\n",k);
        for(nf = 0; nf < nfields; ++nf){
            Ug[k*nfields+nf] = U[0]->gen_aux_field('u');
            Ug[k*nfields+nf]->fhead->type = U[nf]->fhead->type;
         }
    }
fprintf(stdout,"Allocating memory for correlation matrix ....\n");
      // ------------ Calculaing the Correlation matrix ----------
double **C = dmatrix(0,Nsnapshots-1,0,Nsnapshots-1);
for (i=0;i<Nsnapshots; ++i){
    ...
    int ii=0;
for (ig = 0; ig < Nsnapshots/N_Group; ++ig){
    
    for (k =0 ; k < N_Group; ++k) {
        sprintf(f.in.name, "%s_%d.fld", f.rea.name,ii);
        f.in.fp = fopen(f.in.name,"r");
        fprintf(stdout,"Reading i field# %d\n",ii);
        if (!f.in.fp) error_msg(Restart: no dumps read from restart file);
        ReadCopyField(&f,U);
        fclose(f.in.fp);
        ...
        for(nf = 0; nf < nfields; ++nf){
            U[nf]->Trans(U[nf],J_to_Q);
            dsvtvp(U[nf]->htot, -1.0,Mean[nf]->base_h, 1, U[nf]->base_h, 1,U[nf]->base_h, 1 );
            dcopy(U[nf]->htot, U[nf]->base_h, 1, Ug[k*nfields+nf]->base_h, 1);
        }
        ++ii;
    }
   
    for (j = 0; j <= ((ig+1)*N_Group)-1; ++j){
         sprintf(f.in.name, "%s_%d.fld", f.rea.name,j);
         f.in.fp = fopen(f.in.name,"r");
         if (!f.in.fp) error_msg(Restart: no dumps read from restart file);
         ReadCopyField(&f,V);
         fclose(f.in.fp);
        ...
        for(nf = 0; nf < nfields; ++nf){
            V[nf]->Trans(V[nf],J_to_Q);
            dsvtvp(V[nf]->htot, -1.0,Mean[nf]->base_h, 1, V[nf]->base_h, 1,V[nf]->base_h, 1 );
        }
        
        for (k =0 ; k < N_Group; ++k) {
            for(nf = 0; nf < nfields; ++nf){
               dcopy(U[nf]->htot, Ug[k*nfields+nf]->base_h, 1, U[nf]->base_h, 1);
            }
            i = ig*N_Group+k;
            fprintf(stdout, "i=%d, j=%d\n",i,j);
            C[i][j] = compute_InnerProduct(U, V );
...
//}
  ...
  for (i=0; i<Nsnapshots; ++i){
     ...
     // ------------ Calculaing the eigen-values of the Correlation matrix ----------
    double *S_temp;
    double *Sigma;
    double **EigV = dmatrix(0,Nsnapshots-1,0,Nsnapshots-1);
    
    S_temp = dvector(0,Nsnapshots-1);
    Sigma = dvector(0,Nsnapshots-1);
    dzero(Nsnapshots,Sigma,1);
    for (i = 0;  i < Nsnapshots; ++i){
        for (j = 0;  j < Nsnapshots; ++j)
            fprintf(stdout," %f ",i,C[i][j]);
            fprintf(stdout,"  \n");
    }

    //MY_DSYEVD(Nsnapshots, C, S_temp);
    SVD(Nsnapshots, C, Sigma);
    for (i = 0;  i < Nsnapshots; ++i){
        for (j = 0;  j < Nsnapshots; ++j){
            EigV[i][j]=C[j][i];
            fprintf(stdout," %f",i,EigV[i][j]);
        }
//        Sigma[i]=S_temp[Nsnapshots-i-1];
          fprintf(stdout,"  \n");
 //       fprintf(stdout," %f \n",i,Sigma[i]);
//      
    }
    // ------------ Calculaing the POD modes ----------
    fprintf(stdout,"Allocating memory for POD modes --------\n");
    int Np=DIM*U[0]->htot;
    double **PODu = dmatrix(0,N_POD_MODES-1,0,U[0]->htot-1);
    double **PODv = dmatrix(0,N_POD_MODES-1,0,U[0]->htot-1);
    double **PODw = dmatrix(0,N_POD_MODES-1,0,U[0]->htot-1);
   fprintf(stdout, "Calculating POD modes ...........\n"); 
    for (j = 0;  j < Nsnapshots; ++j){
        sprintf(f.in.name, "%s_%d.fld", f.rea.name,j);
        f.in.fp = fopen(f.in.name,"r");
        if (!f.in.fp) error_msg(Restart: no dumps read from restart file);
        ...
        for (i = 0;  i < N_POD_MODES; ++i){
            
            alpha = EigV[j][i]/sqrt(Sigma[i]);
            dsvtvp(U[0]->htot, alpha,U[0]->base_h, 1, PODu[i], 1,PODu[i], 1 );
            dsvtvp(U[0]->htot, alpha,U[1]->base_h, 1, PODv[i], 1,PODv[i], 1 );
            dsvtvp(U[0]->htot, alpha,U[2]->base_h, 1, PODw[i], 1,PODw[i], 1 );
        };
    };
   // free_dmatrix(C,0,0);
   fprintf(stdout,"Writing out POD modes ............\n"); 
     // ------------ Checking the orthogonality/unitary of the POD modes ------------
    for (i=0; i<N_POD_MODES; ++i){
        dcopy(U[0]->htot, PODu[i], 1, U[0]->base_h, 1);
        dcopy(U[0]->htot, PODv[i], 1, U[1]->base_h, 1);
        dcopy(U[0]->htot, PODw[i], 1, U[2]->base_h, 1);
        
        dcopy(U[0]->htot, PODu[1], 1, V[0]->base_h, 1);
        dcopy(U[0]->htot, PODv[1], 1, V[1]->base_h, 1);
        dcopy(U[0]->htot, PODw[1], 1, V[2]->base_h, 1);
        
        
        alpha = compute_InnerProduct(U, V );
        fprintf(stdout,"The inner product between mode %d and 1 = %f\n",i,alpha);
        for(nf = 0; nf < nfields; ++nf)
            U[nf]->Trans(U[nf],Q_to_J);
        
        // ------------ Writeing out POD files ------------
        //sprintf (fname, "/Users/hbabaee/NektarSPM/Utilities/runs/POD_%d.rst",  i);
        sprintf(fname, "%s_POD_%d.fld", f.rea.name,i);
        fp[1] = fp[0] = fopen(fname,"w");
        
        Writefld (fp, "Omega", 0, 0.0, 3, U);
        fprintf(stdout,"Writing out POD mode: %d .......\n",i);
        fclose(fp[0]);
       // fclose(fp[1]);
    }
...
int  setup (FileList *f, Element_List **U, Element_List **V, Element_List **W, int *nft, int Nsnapshots, Bndry *Ubc){
  int    i,shuff;
  int    nfields,nftot;
  Field  fld;
  ...
  for(i = 1; i < nfields; ++i){
    ...
    for(i = 0; i < nfields; ++i){
        V[i] = U[0]->gen_aux_field('u');
        V[i]->fhead->type = fld.type[i];
    }

    for(i = 0; i < nfields; ++i){
        W[i] = U[0]->gen_aux_field('u');
        W[i]->fhead->type = fld.type[i];
    }
    /*
...
void compute_Corr_matrix(double **C, Element_List **E, int field_index, int nfields){
 ...
 double  compute_InnerProduct(Element_List **U, Element_List **V ){
        ...
        int i,j,k,n,nf,qa,qb,qc, p,q,r;
        int qab, qbc, qt;
        double *z,*wa,*wb,*wc,*val,*tmp,*tmp1;
        double *za, *zb, *zc;
        double sum;
        double inner=0;
        Element *F;
        ...
        val = dvector(0,QGmax*QGmax*QGmax-1);
        tmp = dvector(0,QGmax*QGmax*QGmax-1);
        tmp1 = dvector(0,QGmax*QGmax*QGmax-1);
     
//        for (k = 0;k<QGmax*QGmax*QGmax-1; ++k)
//            val[k]=1;
//     
        F = U[0]->flist[0];
        F->GetZW(&za, &wa, &zb, &wb,
        ...
        for (nf = 0; nf < nfields; ++nf){
            ...
            for(k = 0; k < U[0]->nel; ++k){
                ...
                if(F->curvX)
                    dvmul(qt, F->geom->jac.p, 1, tmp1, 1, tmp, 1);
                else
                    dsmul(qt, F->geom->jac.d, tmp1, 1, tmp, 1);
                ...
                inner += ddot(qt,val,1,V[nf]->flist[k]->h_3d[0][0],1);
//                inner += ddot(qt,tmp,1,val,1);
                
            //    fprintf(stdout,"Element number: %d \n",k);
              //  fprintf(stdout,"inner: %f \n",inner);
    ...
    int MY_DSYEVD(int nfields, double **K, double *EIG_VAL){
        ...
        work = new double[1];
        lwork=-1;
        dsyev_( "V", "U", &n, K[0], &n, w, work, &lwork, &info );
        lwork = (int) work[0];
        delete[] work;
    ...
    int SVD(int N, double **C, double *EIG_VAL){
        int m = N, n = N, lda = N, ldu = N, ldvt = N, info, lwork;
        double wkopt;
        double* work;
        double *s;
        /* Local arrays */
        ...
        lwork = -1;
        dgesvd_( "All", "All", &m, &n, C[0], &lda, s, u, &ldu, vt, &ldvt, &wkopt, &lwork,
               &info );
        lwork = (int)wkopt;
        work = (double*)malloc( lwork*sizeof(double) );
        /* Compute SVD */
        dgesvd_( "All", "All", &m, &n, C[0], &lda, s, u, &ldu, vt, &ldvt, work, &lwork,
               &info );
               for (int i=0; i<N; ++i) {
       //     fprintf(stdout,"s[%d]=%f, EIG_VAL[%d]=%f\n",i,s[i],i,EIG_VAL[i]);
