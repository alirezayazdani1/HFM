 ...
 *                        RCS Information                                    *
 *                                                                           *
 * $Source: /homedir/cvs/Nektar/Utilities/src/grad.C,v $
 * $Revision: 1.2 $
 * $Date: 2005/08/12 10:18:05 $ 
 * $Author: ssherw $ 
 * $State: Exp $ 
 *---------------------------------------------------------------------------*/
#include <stdio.h>
...
static void Calc_Grad_Lap (FileList *f, Element_List **U, int nfields);
...
main (int argc, char *argv[])
{
  ...
  Calc_Grad_Lap(&f,master,nfields);
...
static void Calc_Grad_Lap (FileList *f, Element_List **U, int nfields){
  ...
  if(U[0]->fhead->dim() == 3){
    ...
    for(k = 0; k < U[0]->nel; ++k){
      ...
      dcopy(qt,Ux,1,*U[nfields-1]->flist[k]->h,1);
      ...
      dcopy(qt,Uy,1,*U[nfields+2]->flist[k]->h,1);
      U[nfields+2]->flist[k]->Grad_d(0, Uyy , 0, 'y');
      dcopy(qt,Uyy,1,*U[nfields+2]->flist[k]->h,1);
